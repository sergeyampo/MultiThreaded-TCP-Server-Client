...
uint64_t naive_factorial(uint64_t n)
{
    int64_t r = 1;
    for (int i = 2; i <= n; ++i)
        r *= i;
...
void * socket_thread(void * arg){
    ...
    while (true) {
        ...
        unsigned int buffer_size = sizeof(uint64_t) * 2;
        ...
        memcpy(&mod, from_client +  sizeof(uint64_t), sizeof(uint64_t));

        fprintf(stdout, "Receive: %llu %llu %llu\n", factorial_number, mod);
        uint64_t total = naive_factorial(factorial_number);
        printf("Total: %llu\n", total);
